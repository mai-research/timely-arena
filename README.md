...
The two curated battles show a two-turn graph exploration comparison and a single-pass LLM/graph comparison with a recovered evidence-query timeout. Each trace has a specific purpose, named node references, source evidence, and case-preservation checks. Failed attempts and retries keep separate call IDs. Single pass means one user prompt, not one agent query. Candidate mappings, outputs, statuses and timestamps are fixed. Execution details appear before each final answer; the optional inspector has independent A/B turn and step cursors, cumulative prefixes, and no playback. Query and return events are separate. Missing traces or non-graph methods are N/A, and mismatched graph versions do not contribute to coverage.
